                      ...
                      <div class="col-md-12">
                          <div class="card">
                              <div class="card-header">
                                <img class="card-img-top img-responsive img-thumbnail" src="{{ asset('img/cards/category_pesticide_plant.png')}}"  style="height: 50px; width: 50px;"alt="Imagem" >
                                Tipos de Conta
                                <a class="float-right" href="{{url('category_pesticide/create')}}">Cadastrar</a>
                              </div>
                          </div>
                      </div>
                  </div>
                  ...
                  <table id="example1" class="table table-sm table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                    ...
                    <tbody>
                      ...
                      @foreach($category_pesticides as $category_pesticide)
                        ...
                        <tr>
                          <td>  
                          <a href= "{{ route('category_pesticide.edit' ,[ 'category_pesticide' => $category_pesticide->id  ])}}" >{{ $category_pesticide->name}}</a>
                          </td>
                          <td>  
                            <a href= "{{ route('category_pesticide.edit' ,[ 'category_pesticide' => $category_pesticide->id  ])}}" >{{ $category_pesticide->description}}</a>
                          </td>  
            
                            <form id="delete-form"  method="POST" action="{{ route('category_pesticide.destroy' ,[ 'category_pesticide' => $category_pesticide->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
