                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            Tipo de atividades
                           <a class="float-right" href="{{url('bayer/create')}}">Cadastrar</a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                      ...
                      @foreach($bayers as $bayer)
                        ...
                        <tr>
                          ...
                          <td>  
                          <a href= "{{ route('bayer.edit' ,[ 'bayer' => $bayer->id  ])}}" >{{ $bayer -> name}}</a>
                          </td>
                          <td>
                            <textarea class="form-control" rows="2" cols= "33" > {{$bayer->note }} </textarea>
                          </td>
        
                          <td >
            
                            <form id="delete-form"  method="POST" action="{{ route('bayer.destroy' ,[ 'bayer' => $bayer->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
