...
<head>
    ...
    <title>FC - Tipo de documento</title>
                  ...
                  <table id="example1" class="table table-sm table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                      <thead>
                          <tr>
                              ...
                              <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">Tipo de documento</th>
                    ...
                    <tbody>
                      @foreach($banks as $bank)
                        ...
                        <tr>
                          ...
                          <td>  
                          <a href= "{{ route('bank.edit' ,[ 'bank' => $bank->id  ])}}" >{{ $bank -> name}}</a>
                          </td>
                          <td>
                            <textarea class="form-control" rows="2" cols= "33" > {{$bank->note }} </textarea>
                          </td>
        
                          <td >
            
                            <form id="delete-form"  method="POST" action="{{ route('bank.destroy' ,[ 'bank' => $bank->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
