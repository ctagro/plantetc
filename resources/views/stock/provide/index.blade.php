                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            Tipo de atividades
                           <a class="float-right" href="{{url('provide/create')}}">Cadastrar</a>
                        </div>
                    </div>
                </div>
            </div>
                  ...
                  <table id="example1" class="table table-sm table-bordered table-striped dataTable dtr-inline collapsed" role="grid" aria-describedby="example1_info">
                      <thead>
                          <tr>
                              ...
                              <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">Vendedor</th>
                    ...
                    <tbody>
                      @foreach($provides as $provide)
                        ...
                        <tr>
                          ...
                          <td>  
                          <a href= "{{ route('provide.edit' ,[ 'provide' => $provide->id  ])}}" >{{ $provide -> name}}</a>
                          </td>
                          <td>  
                            <a href= "{{ route('provide.edit' ,[ 'provide' => $provide->id  ])}}" >{{ $provide -> salesman}}</a>
                            </td>
                          <td>
                            <textarea class="form-control" rows="2" cols= "33" > {{$provide->note }} </textarea>
                          </td>
        
                          <td >
            
                            <form id="delete-form"  method="POST" action="{{ route('provide.destroy' ,[ 'provide' => $provide->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
