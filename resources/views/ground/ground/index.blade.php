                      ...
                      <div class="col-md-12">
                          <div class="card">
                              <div class="card-header">
                                ...
                                Área de Plantio
                                <a class="float-right" href="{{url('ground/create')}}">Cadastrar</a>
                              </div>
                          </div>
                      </div>
                  </div>
                    ...
                    <tbody>
                      ...
                      @foreach($grounds as $ground)
                        ...
                        <tr>
                          ...
                          <td>  
                          <a href= "{{ route('ground.edit' ,[ 'ground' => $ground->id  ])}}" >{{ $ground->name}}</a>
                          </td>
                          <td>  
                            <a href= "{{ route('ground.edit' ,[ 'ground' => $ground->id ])}}" >{{ number_format($ground->area, 2 , ',', '.')  }}</a>
                          </td>
                          <td>  
                            <a href= "{{ route('ground.edit' ,[ 'ground' => $ground->id  ])}}" >{{ $ground->location}}</a>
                            </td>
                           
        
                          <td >
            
                            <form id="delete-form"  method="POST" action="{{ route('ground.destroy' ,[ 'ground' => $ground->id ])}}", style = 'display: inline;'> 
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}                 
                            </form>
                          </td>
